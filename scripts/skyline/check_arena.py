...
import gzip,struct,json,collections
root=Path(__file__).resolve().parents[2];d=root/'data/skyline'
b=gzip.decompress((d/'heights.mpz').read_bytes());off=0
def get(fmt):
 global off
...
for _ in range(nents):
 x,y,z,t=get('<3fB3x');assert t==3
 n=get('<i')[0];assert n==7;off+=n*4;assert get('<i')[0]==0
...
layout=json.loads((d/'layout.json').read_text());boxes=layout['boxes']
...
print(f'PASS MAPZ: {leaves} leaves, {nents} clear spawns, exact byte consumption')
